...
spawn(function()
    local KEY = "theuslindo"  -- Substitua ReadKey() pela chave fixa
    if KEY and #KEY >= 2 then
        object.Desc.Text = 'Successfully loaded key!'
        Notify.New("Successfully loaded key!", 5)
        wl(KEY)
    end
end)

object.CheckKey.MouseButton1Click:Connect(function()
    -- Remova a verificação de GetKeyText() e use a chave fixa
    local keyText = "theuslindo"  -- Chave fixa
    if not keyText or #keyText < 10 then
    ...
    wl(keyText)  -- Use a chave fixa
end)

object.GetKey.MouseButton1Click:Connect(function()
    print('get key')
    -- Você pode remover esta parte ou deixá-la como está
