...
addToggle("Safe Aimbot (TS)", false, function() end)
...
task.spawn(function()
    while true do
        task.wait(0.5)
        if toggles["Safe Aimbot (TS)"]() then
            local char = LocalPlayer.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                local closestTitan, minDist = nil, math.huge
                for _, titan in pairs(workspace:GetDescendants()) do
                    if titan:FindFirstChild("Humanoid") and titan:FindFirstChild("Nape") then
                        local dist = (char.HumanoidRootPart.Position - titan.Nape.Position).Magnitude
                        if dist < minDist then
                            closestTitan, minDist = titan, dist
                        end
                    end
                end
                if closestTitan then
                    -- Ajusta a posição do jogador para mirar na nuca do Titan
                    local targetPos = closestTitan.Nape.Position + Vector3.new(0, 8, 0)
                    char.HumanoidRootPart.CFrame = CFrame.new(targetPos, closestTitan.Nape.Position)
                    -- Aqui você pode adicionar o comando para disparar a Thunder Spear, se necessário
                end
            end
        end
    end
